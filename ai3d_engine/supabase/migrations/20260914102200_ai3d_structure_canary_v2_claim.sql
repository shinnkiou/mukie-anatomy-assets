-- NEVER TEAR AI3D-013 bounded retry identity after CANARY-001 diagnostic hold.
-- CANARY-001 reached the correct Structure claim but did not produce durable
-- physical evidence. Preserve it as diagnostic history and use a new exact key.

create or replace function public.ai3d_claim_structure_canary_job_v2(
  p_device_key text,
  p_lease_owner text,
...
begin
  ...
  where j.status = 'QUEUED'
    ...
    and j.job_key = 'AI3D-013-PHYSICAL-STRUCTURE-CANARY-002'
    and j.command_id = 'AI3D-013-STRUCTURE-CANARY-002-NEVER-TEAR'
...
revoke all on function public.ai3d_claim_structure_canary_job_v2(text, text, integer)
  from public, anon, authenticated;
grant execute on function public.ai3d_claim_structure_canary_job_v2(text, text, integer)
  to service_role;

comment on function public.ai3d_claim_structure_canary_job_v2(text, text, integer) is
  'Fail-closed exact claim path for AI3D-013 Structure physical CANARY-002 only.';
