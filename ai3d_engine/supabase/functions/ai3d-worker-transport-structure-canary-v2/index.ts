...
const WORKER_VERSION = "0.2.1-ai3d-structure-diagnostic";
const ACTION = "ai3d_structure_module_v1";
const TASK_ID = "AI3D-013";
const JOB_KEY = "AI3D-013-PHYSICAL-STRUCTURE-CANARY-002";
const COMMAND_ID = "AI3D-013-STRUCTURE-CANARY-002-NEVER-TEAR";
...
async function identity(req: Request) {
  ...
  const { data, error } = await admin.from("ukie_worker_devices").select("id,device_key,status,metadata").eq("device_key",key).eq("secret_hash",secret).eq("status","ACTIVE").maybeSingle();
  return error || !data ? null : data;
}
function exactCompanion(body: any) {
...
async function touch(worker: any) {
  const now = new Date().toISOString();
  const metadata = { ...(worker.metadata || {}), ai3d_worker_version: WORKER_VERSION, ai3d_contract_version: PROTOCOL, ai3d_capabilities: [ACTION], ai3d_last_seen: now, ai3d_structure_canary_only: true, ai3d_diagnostic_generation: 2 };
...
async function armOnce() {
  ...
  if (data && !error) {
    await admin.from("ai3d_worker_events").insert({job_id:data.id,event_type:"STRUCTURE_CANARY_V2_AUTO_ARMED",status:"QUEUED",detail:{worker_version:WORKER_VERSION,capability:ACTION,prior_canary:"AI3D-013-PHYSICAL-STRUCTURE-CANARY-001"}});
    return data;
  }
  ...
  throw new Error(`canary_v2_arm_failed:${error?.code || "unknown"}`);
}
async function lease(jobId: string, owner: string, deviceKey: string) {
  ...
  if (job.status!=="RUNNING" || String(job.lease_owner||"")!==owner) return {error:"stale_lease"};
  if (!job.lease_until || new Date(job.lease_until).getTime()<Date.now()) return {error:"lease_expired"};
  return {job};
}
function cleanCompletion(body: any) {
  const r = body?.result || {};
  ...
  return {rawPixelSha:String(r.raw_pixel_sha256).toLowerCase(),pngSha:String(r.png_sha256).toLowerCase(),checkpointSha:String(r.checkpoint_sha256).toLowerCase(),checkpointId,renderer:String(r.renderer||"").slice(0,160),blenderVersion:String(r.blender_version||"").slice(0,80),renderEngine:String(r.render_engine||"").slice(0,80),booleanSolver:String(r.boolean_solver||"").slice(0,40),width:Number(r.width),height:Number(r.height)};
}
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "PUT") {
    ...
    const active=await lease(jobId,owner,worker.device_key); if(!active.job) return reply({ok:false,error:active.error},409);
    ...
    const uploadSha=await shaBytes(bytes); if(uploadSha!==expected) return reply({ok:false,error:"client_sha_mismatch"},409);
    const storagePath=`structure_canary_v2/jobs/${jobId}/ai3d_structure_capability_canary.png`;
    ...
    const {data:blob,error:dlErr}=await admin.storage.from(BUCKET).download(storagePath); if(dlErr||!blob) return reply({ok:false,error:"storage_readback_failed"},500);
    const readbackSha=await shaBytes(await blob.arrayBuffer()); if(readbackSha!==uploadSha) return reply({ok:false,error:"storage_readback_sha_mismatch"},500);
    const artifactId=`structure-canary-v2-render-${uploadSha.slice(0,16)}`;
    const artifact={artifact_id:artifactId,project_key:"never_tear_ai3d_engine",task_id:TASK_ID,category:"PIXEL_RENDER_EVIDENCE",sha256:uploadSha,provider:"SUPABASE_STORAGE",uri:`supabase://${BUCKET}/${storagePath}`,qa_status:"AUTOMATED_PENDING",readback_status:"PASS",metadata:{worker_job_id:jobId,action:ACTION,evidence_class:"PHYSICAL_WINDOWS_BLENDER_STRUCTURE_CAPABILITY_CANARY_V2",storage_bucket:BUCKET,storage_path:storagePath,png_sha256:uploadSha,size_bytes:bytes.byteLength,provider_readback_sha256:readbackSha,visual_qa:"PENDING",canary_promoted:false}};
    const {error:artErr}=await admin.from("ai3d_artifacts").upsert(artifact,{onConflict:"artifact_id"}); if(artErr) return reply({ok:false,error:"artifact_ledger_write_failed"},500);
    const now=new Date().toISOString();
    ...
    await admin.from("ai3d_worker_events").insert({job_id:jobId,event_type:"STRUCTURE_CANARY_V2_ARTIFACT_READBACK_PASS",status:"RUNNING",device_key:worker.device_key,detail:{artifact_id:artifactId,png_sha256:uploadSha,size_bytes:bytes.byteLength}});
    await touch(worker);
  ...
  const length=Number(req.headers.get("content-length")||"0"); if(length>MAX_JSON_BYTES) return reply({ok:false,error:"body_too_large"},413);
  ...
  if(body.action==="heartbeat"){
    const now=await touch(worker); const jobId=String(body.job_id||""); const owner=String(body.lease_owner||"");
    if(jobId&&owner){
      const active=await lease(jobId,owner,worker.device_key); if(!active.job) return reply({ok:false,error:active.error},409);
      ...
      await admin.from("ai3d_worker_events").insert({job_id:jobId,event_type:"STRUCTURE_CANARY_V2_HEARTBEAT",status:"RUNNING",device_key:worker.device_key,detail:{lease_until:until,worker_version:WORKER_VERSION,capability:ACTION}});
      return reply({ok:true,status:"RUNNING",lease_until:until,protocol_version:PROTOCOL});
    }
    try{const canary=await armOnce();return reply({ok:true,status:"ONLINE",server_time:now,protocol_version:PROTOCOL,canary_job_key:JOB_KEY,canary_job_status:canary.status});}
    catch(e){return reply({ok:false,error:e instanceof Error?e.message:"canary_v2_arm_failed"},500);}
  }

  if(body.action==="claim"){
    const owner=`${worker.device_key}:${crypto.randomUUID()}`;
    const {data,error}=await admin.rpc("ai3d_claim_structure_canary_job_v2",{p_device_key:worker.device_key,p_lease_owner:owner,p_lease_seconds:LEASE_SECONDS});
    if(error) return reply({ok:false,error:"structure_claim_v2_failed"},500);
  ...
  if(body.action==="complete"){
    const jobId=String(body.job_id||""); const owner=String(body.lease_owner||""); const outcome=String(body.outcome||"");
    if(!jobId||owner.length<8||!["PASS","FAIL"].includes(outcome)) return reply({ok:false,error:"completion_invalid"},400);
    const active=await lease(jobId,owner,worker.device_key); if(!active.job) return reply({ok:false,error:active.error},409);
    const now=new Date().toISOString();
    if(outcome==="FAIL"){
      const code=String(body.error_code||"STRUCTURE_CANARY_V2_FAILED").slice(0,120); const message=String(body.error_message||"").slice(0,1000);
      ...
      await admin.from("ai3d_worker_events").insert({job_id:jobId,event_type:"STRUCTURE_CANARY_V2_FAILED",status:"FAIL",device_key:worker.device_key,detail:{error_code:code,error_message:message.slice(0,240)}});
      await admin.from("ai3d_tasks").update({status:"ACTIVE",stage:"QA_FAIL_FIX",next_action:"Read CANARY-002 server-side failure detail, fix the smallest cause, preserve evidence, and do not unblock AI3D-010.",updated_at:now}).eq("task_id",TASK_ID);
      return reply({ok:true,status:"FAIL",job_key:JOB_KEY,completed_at:now});
    }
    let c:any; try{c=cleanCompletion(body);}catch(e){return reply({ok:false,error:e instanceof Error?e.message:"completion_result_invalid"},409);}
    const saved=active.job.result?.artifact||{}; if(saved.storage_readback_status!=="PASS"||saved.png_sha256!==c.pngSha||!saved.artifact_id) return reply({ok:false,error:"durable_artifact_readback_required"},409);
    ...
    const {error:cpErr}=await admin.from("ai3d_checkpoints").upsert({checkpoint_id:c.checkpointId,project_key:"never_tear_ai3d_engine",task_id:TASK_ID,run_key:`AI3D-WORKER-${JOB_KEY}`,sha256:c.checkpointSha,state_json:checkpointState,next_action:"Perform visual QA on the durable AI3D-013 Structure CANARY-002 PNG. Promote only if visual QA also passes."},{onConflict:"checkpoint_id"}); if(cpErr) return reply({ok:false,error:"checkpoint_ledger_write_failed"},500);
    const runKey=`AI3D-WORKER-${JOB_KEY}`;
    const {error:runErr}=await admin.from("ai3d_runs").upsert({run_key:runKey,project_key:"never_tear_ai3d_engine",task_id:TASK_ID,status:"CANARY_AUTOMATED_PASS_VISUAL_PENDING",route:"WINDOWS_WORKER_BLENDER_STRUCTURE_CANARY_V2",command_plan:PLAN,qa_report:{automated_qa:"PASS",storage_readback:"PASS",visual_qa:"PENDING",artifact_id:saved.artifact_id,wall_count:1,opening_boolean_count:1},checkpoint_id:c.checkpointId,error_summary:null,started_at:active.job.started_at||now,finished_at:now},{onConflict:"run_key"}); if(runErr) return reply({ok:false,error:"run_ledger_write_failed"},500);
    ...
    const {error:jobErr}=await admin.from("ai3d_worker_jobs").update({status:"PASS",result:finalResult,completed_at:now,lease_until:null,updated_at:now,error_code:null,error_detail:{}}).eq("id",jobId).eq("lease_owner",owner); if(jobErr) return reply({ok:false,error:"job_completion_write_failed"},500);
    const {data:task}=await admin.from("ai3d_tasks").select("artifact_refs,qa_summary").eq("task_id",TASK_ID).maybeSingle(); const refs=Array.isArray(task?.artifact_refs)?task.artifact_refs.map(String):[]; if(!refs.includes(saved.artifact_id)) refs.push(saved.artifact_id);
    await admin.from("ai3d_tasks").update({status:"ACTIVE",stage:"QA",artifact_refs:refs,qa_summary:{...(task?.qa_summary||{}),structure_canary_v2:{automated_qa:"PASS",storage_readback:"PASS",visual_qa:"PENDING",artifact_id:saved.artifact_id,checkpoint_id:c.checkpointId,structure_module_capability_promoted:false}},next_action:"Perform visual QA on AI3D-013 Structure CANARY-002. Only on PASS promote ai3d_structure_module_v1, complete RIO-60, and unblock AI3D-010.",updated_at:now}).eq("task_id",TASK_ID);
    await admin.from("ai3d_projects").update({next_action:"AI3D-013 Structure CANARY-002 automated QA passed; visual QA is required before promotion or AI3D-010 production.",updated_at:now}).eq("project_key","never_tear_ai3d_engine");
    await admin.from("ai3d_worker_events").insert({job_id:jobId,event_type:"STRUCTURE_CANARY_V2_AUTOMATED_PASS",status:"PASS",device_key:worker.device_key,detail:{artifact_id:saved.artifact_id,checkpoint_id:c.checkpointId,visual_qa:"PENDING",canary_promoted:false}});
    await touch(worker);
