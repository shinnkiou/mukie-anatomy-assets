...
def build(width: int = 12, height: int = 8, length: int = 6):
    ...
    for y in range(1, 7):
        ...
        for x in range(width):
            put(x, y, 0, wall); put(x, y, length-1, wall)
        for z in range(1, length-1):
            put(0, y, z, wall); put(width-1, y, z, wall)
    ...
    for x in (3,4,7,8,9):
        put(x, 1, 0, "minecraft:glass"); put(x, 2, 0, "minecraft:glass")
    for x in (2,3,5,6,8,9):
        put(x, 4, 0, "minecraft:glass"); put(x, 5, 0, "minecraft:glass")
    for x in (2,3,7,8):
        put(x, 4, length-1, "minecraft:glass"); put(x, 5, length-1, "minecraft:glass")
    for z in (2,3):
        put(0, 4, z, "minecraft:glass"); put(width-1, 4, z, "minecraft:glass")

    for x in range(width):
        put(x, 3, 0, "minecraft:polished_andesite_slab[type=bottom,waterlogged=false]")
    for x in range(4, 8):
        put(x, 3, 0, "minecraft:dark_oak_sign[rotation=0,waterlogged=false]")
    ...
    for y in range(height):
        for z in range(length):
            for x in range(width):
                ids.append(palette.get(states.get((x,y,z), "minecraft:air"), 0))
    ...
    root = Compound({
        ...
        "Metadata": Compound({
            "Name": String("NEVER_TEAR_VOXEL_FIXTURE_R1"),
            "Author": String("ChatGPT Blender integration project"),
            "License": String("CC0-1.0 project-owned fixture"),
        }),
    })
