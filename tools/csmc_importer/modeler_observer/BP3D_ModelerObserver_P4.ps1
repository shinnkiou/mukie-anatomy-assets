...
function New-Root{
  $c=@()
  if($env:USERPROFILE){$d=Join-Path $env:USERPROFILE 'Desktop';if(Test-Path $d){$c+=Join-Path $d 'BP3D_ModelerObserver_P4_Captures'}}
  if($env:TEMP){$c+=Join-Path $env:TEMP 'BP3D_ModelerObserver_P4_Captures'}
...
$Dir=Join-Path $Root ('CAPTURE_'+$Stamp+'_P4');New-Item -ItemType Directory -Force -Path $Dir|Out-Null
$Log=Join-Path $Dir 'observer_p4.log.txt'
...
Log START "version=$ToolVersion expected_guid=$ExpectedGuid saved_logical=$SavedLogical saved_stored=$SavedStored safe=$SafeMode max_scan_mib=$MaxScanMiB"
...
using System.Text;
public static class BP3DTargetDumpP4 {
 const uint VM=0x10,QI=0x400,QLI=0x1000,COMMIT=0x1000,NO=1,GUARD=0x100,MEM_PRIVATE=0x20000;
 ...
 public class Cand{public ulong BlobStart,MagicAddress,HeaderBytes,Stored,Logical,Total;public uint Version;public string Guid,Kind;public uint Protect,Type;public long StoredMinusAlign8Logical;public bool MemPrivate,InvariantOk,FullHeaderValidated;}
 ...
 static List<MBI> Plan(IntPtr hp,ulong max,out ulong total){
   ...
   while(a<0x00007FFFFFF00000UL&&total<max){MBI m;var q=VirtualQueryEx(hp,new IntPtr(unchecked((long)a)),out m,new UIntPtr((uint)ms));if(q==UIntPtr.Zero){a+=0x10000;continue;}
    ...
    if(RP(m)){ulong take=Math.Min(rs,max-total);if(take>0){m.RegionSize=new UIntPtr(take);z.Add(m);total+=take;}}a=nx;}
   return z;
 }
 ...
 static Cand Parse(IntPtr hp,ulong magic,string expected,uint prot,uint type){
   ...
   ulong start=magic-4;byte[] h=new byte[128];if(!ReadAt(hp,start,h,h.Length))return null;
   ...
   ulong aligned=Align8(logical);long relation=(long)((ulong)stored-aligned);bool invariant=relation==8;
   if(!invariant)return null;
   return new Cand{BlobStart=start,MagicAddress=magic,HeaderBytes=(ulong)o,Stored=stored,Logical=logical,Total=(ulong)o+stored,Version=ver,Guid=guid,Kind=kind,Protect=prot,Type=type,StoredMinusAlign8Logical=relation,MemPrivate=true,InvariantOk=true,FullHeaderValidated=true};
 }
 public static SearchResult Search(int pid,string expected,ulong max,int timeout){
   ...
   var sw=Stopwatch.StartNew();try{ulong planned;var regs=Plan(hp,max,out planned);r.Planned=planned;r.Regions=regs.Count;byte[] pat=Encoding.ASCII.GetBytes("CLIP_STUDIO_3D_DATA2"),buf=new byte[1024*1024];ulong attempted=0,good=0;int fail=0,last=-1;
    Console.WriteLine(String.Format("[P4] MEM_PRIVATE search plan {0:F1} MiB / {1} regions",planned/1048576.0,regs.Count));
    foreach(var m in regs){ulong b=unchecked((ulong)m.BaseAddress.ToInt64()),sz=m.RegionSize.ToUInt64(),off=0;while(off<sz){
      ...
      int pct=planned==0?100:(int)Math.Min(100UL,attempted*100UL/planned);if(pct!=last){P("P4-search",attempted,planned,good,outp.Count,fail,sw,false);last=pct;}off+=(ulong)want;}}
    r.Reason="complete";
   done:r.Attempted=attempted;r.Read=good;r.Failures=fail;r.Candidates=outp.ToArray();r.Seconds=sw.Elapsed.TotalSeconds;P("P4-search",attempted,planned,good,outp.Count,fail,sw,true);return r;
   }finally{sw.Stop();CloseHandle(hp);}
 }
 public static DumpResult Dump(int pid,Cand c,string path,int timeout){
   var r=new DumpResult();r.Planned=c.Total;r.Path=path;r.Reason="not_started";if(!c.MemPrivate||!c.InvariantOk||!c.FullHeaderValidated){r.Reason="candidate_not_fail_closed_validated";return r;}
   IntPtr hp=OpenProcess(VM|QI,false,pid);if(hp==IntPtr.Zero)hp=OpenProcess(VM|QLI,false,pid);if(hp==IntPtr.Zero){r.Reason="open_failed";return r;}
   var sw=Stopwatch.StartNew();ulong pos=0;int fail=0;try{using(var fs=new FileStream(path,FileMode.Create,FileAccess.Write,FileShare.None)){byte[] buf=new byte[1024*1024];while(pos<c.Total){
     if(timeout>0&&sw.Elapsed.TotalSeconds>=timeout){r.Reason="timeout";break;}ulong a=c.BlobStart+pos;MBI m;int ms=Marshal.SizeOf(typeof(MBI));if(VirtualQueryEx(hp,new IntPtr(unchecked((long)a)),out m,new UIntPtr((uint)ms))==UIntPtr.Zero||!RP(m)){r.Reason="non_private_or_unreadable_region";fail++;break;}
     ulong rb=unchecked((ulong)m.BaseAddress.ToInt64()),re=rb+m.RegionSize.ToUInt64();ulong room=re>a?re-a:0;if(room==0){r.Reason="zero_room";fail++;break;}int want=(int)Math.Min((ulong)buf.Length,Math.Min(c.Total-pos,room));UIntPtr g;bool ok=ReadProcessMemory(hp,new IntPtr(unchecked((long)a)),buf,new UIntPtr((uint)want),out g);int n=ok?(int)Math.Min((ulong)want,g.ToUInt64()):0;if(n<=0){r.Reason="read_failed";fail++;break;}fs.Write(buf,0,n);pos+=(ulong)n;P("P4-dump",pos,c.Total,pos,0,fail,sw,false);
   }}if(pos==c.Total)r.Reason="complete";r.Written=pos;r.Failures=fail;r.Seconds=sw.Elapsed.TotalSeconds;P("P4-dump",pos,c.Total,pos,0,fail,sw,true);
...
$Interop=$false
if(-not$SafeMode){try{Add-Type -TypeDefinition $code -Language CSharp -ErrorAction Stop;$Interop=$true;Log INTEROP_READY 'P4 fail-closed reader compiled.'}catch{Log INTEROP_FAILED $_.Exception.ToString()}}
Write-Host ''
Write-Host 'BP3D MODELER OBSERVER P4' -ForegroundColor Cyan
Write-Host 'Read-only; scans committed readable MEM_PRIVATE only.'
Write-Host 'Candidate gate: full CELSYS character header + exact GUID + v2 + stored/alignment invariant.'
...
Write-Host ''
Write-Host 'Keys: B=validated MEM_PRIVATE character capture  Q=finish/package' -ForegroundColor DarkCyan
...
while(-not$done){
  ...
  if([Console]::KeyAvailable){$k=[Console]::ReadKey($true).KeyChar.ToString()
    ...
    elseif($k-match'[bB]'){
      ...
      Log BLOB_SEARCH_START "pid=$($p.Id) mem_private=true"
      $maxBytes=[UInt64]$MaxScanMiB*1MB
      $s=[BP3DTargetDumpP4]::Search($p.Id,$ExpectedGuid,$maxBytes,$SearchTimeoutSec);$last=@($s.Candidates)
      $cand=@();$i=0
      foreach($c in $last){
        $i++
        $logicalDelta=[Int64]$c.Logical-[Int64]$SavedLogical
        $storedDelta=[Int64]$c.Stored-[Int64]$SavedStored
        $cand+=[ordered]@{index=$i;blob_start=('0x{0:X}'-f$c.BlobStart);magic_address=('0x{0:X}'-f$c.MagicAddress);guid=$c.Guid;kind=$c.Kind;version=$c.Version;logical=$c.Logical;stored=$c.Stored;header_bytes=$c.HeaderBytes;total=$c.Total;protect=('0x{0:X}'-f$c.Protect);type=('0x{0:X}'-f$c.Type);mem_private=$c.MemPrivate;full_header_validated=$c.FullHeaderValidated;invariant_ok=$c.InvariantOk;stored_minus_align8_logical=$c.StoredMinusAlign8Logical;logical_delta_vs_saved=$logicalDelta;stored_delta_vs_saved=$storedDelta}
      }
      $meta=[ordered]@{tool_version=$ToolVersion;pid=$p.Id;process=$p.ProcessName;exe=(ProcPath $p);expected_guid=$ExpectedGuid;saved_logical=$SavedLogical;saved_stored=$SavedStored;mem_private_required=$true;full_header_required=$true;stored_alignment_relation_required=8;search_reason=$s.Reason;search_seconds=$s.Seconds;search_planned=$s.Planned;search_attempted=$s.Attempted;search_read=$s.Read;search_failures=$s.Failures;candidates=$cand}
      ...
      if($last.Count-eq0){Write-Host '[P4] No fail-closed MEM_PRIVATE character candidate found.' -ForegroundColor Yellow;continue}
      if($last.Count-ne1){Write-Host ('[P4] Ambiguous: '+$last.Count+' candidates. No automatic dump. Package metadata and inspect first.') -ForegroundColor Yellow;Log BLOB_DUMP_SKIPPED 'ambiguous_candidate_count';continue}
      Write-Host '[P4] Exactly one validated MEM_PRIVATE candidate.' -ForegroundColor Green
      $c=$last[0];$out=Join-Path $Dir ('runtime_character_PRIVATE_0x{0:X}.bin'-f$c.BlobStart)
      Write-Host ('[P4] Read-only dump: '+$c.Total+' bytes') -ForegroundColor Cyan
      $d=[BP3DTargetDumpP4]::Dump($p.Id,$c,$out,$DumpTimeoutSec)
      $dm=[ordered]@{reason=$d.Reason;planned=$d.Planned;written=$d.Written;failures=$d.Failures;seconds=$d.Seconds;sha256=$d.Sha256;path=[IO.Path]::GetFileName($d.Path);private_payload=$true;automatic_network_upload=$false}
      ...
      if($d.Reason-eq'complete'){Write-Host ('[P4] COMPLETE 100% SHA-256 '+$d.Sha256) -ForegroundColor Green;Write-Host 'Keep the runtime .bin/ZIP private; do not publish it to GitHub.' -ForegroundColor Yellow}else{Write-Host ('[P4] PARTIAL reason='+$d.Reason) -ForegroundColor Yellow}
      Write-Host 'Observer is idle again. Press Q to package local output.' -ForegroundColor DarkCyan
    }
  }
  Start-Sleep -Milliseconds 200
}

$manifest=[ordered]@{tool='BP3D Modeler Observer P4';version=$ToolVersion;expected_guid=$ExpectedGuid;read_only=$true;mem_private_required=$true;full_header_required=$true;stored_alignment_relation_required=8;automatic_network_upload=$false;contains_private_runtime_payload=([bool](Get-ChildItem -Path $Dir -Filter 'runtime_character_PRIVATE_*.bin' -ErrorAction SilentlyContinue));target_pid=if($p){$p.Id}else{0};target_executable=if($p){ProcPath $p}else{''};end=(Get-Date).ToString('o')}
$manifest|ConvertTo-Json|Set-Content (Join-Path $Dir 'capture_manifest.json') -Encoding UTF8
$zip=$Dir+'.zip'
try{Compress-Archive -Path (Join-Path $Dir '*') -DestinationPath $zip -Force;Write-Host ('PRIVATE OUTPUT ZIP: '+$zip) -ForegroundColor Green;Write-Host 'Attach this ZIP privately to the project chat; do not commit it to public GitHub.' -ForegroundColor Yellow}catch{Write-Host ('ZIP failed; keep folder private: '+$Dir) -ForegroundColor Yellow}
Write-Host ''
