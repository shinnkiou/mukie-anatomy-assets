...
def self_test() -> None:
    ...
    bad = dict(base, semantic_promotion_count=1)
    assert validate(bad)["accepted"] is False
    bad = dict(base, blender_mesh_emit_ready=True)
    assert validate(bad)["accepted"] is False
    bad = dict(base, merge_policy="AUTO_MERGE")
    assert validate(bad)["accepted"] is False
    bad = dict(base, isolation={**base["isolation"], "runtime_jobs": 1})
    assert validate(bad)["accepted"] is False
