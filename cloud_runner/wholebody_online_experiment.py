# -*- coding: utf-8 -*-
"""BP3D public-safe online experiment. Uses only MakeHuman CC0 base.obj."""
import bpy,sys,os,json,math,hashlib,traceback
from pathlib import Path
from collections import Counter,defaultdict,deque
from array import array
from mathutils import Vector

MODES=('D2','E2','F2'); SIZE=1024
...
def rgb(i):
 if i>=100:return(.94,.94,.94,1)
 import colorsys;return (*colorsys.hsv_to_rgb((i*.61803398875)%1,.68,.92),1)
def mat(name,c):
 m=bpy.data.materials.new(name);m.diffuse_color=c;m.use_nodes=True;nt=m.node_tree;nt.nodes.clear();o=nt.nodes.new('ShaderNodeOutputMaterial');e=nt.nodes.new('ShaderNodeEmission');e.inputs['Color'].default_value=c;e.inputs['Strength'].default_value=1;nt.links.new(e.outputs['Emission'],o.inputs['Surface']);return m

def comp(mesh):
 v2=[[] for _ in mesh.vertices]
 for p in mesh.polygons:
  for v in p.vertices:v2[v].append(p.index)
 seen=bytearray(len(mesh.polygons));cs=[]
 for s in range(len(mesh.polygons)):
  ...
  cs.append(c)
 cs.sort(key=len,reverse=True);return cs

def clean(src,ids):
 sm=src.data;m=bpy.data.meshes.new('BP3D_CLEAN');m.from_pydata([v.co.copy() for v in sm.vertices],[],[list(sm.polygons[i].vertices) for i in ids]);m.update()
 ...
 o=bpy.data.objects.new('BP3D_CLEAN_REFERENCE',m);bpy.context.scene.collection.objects.link(o);return o

def norm(o):
 used=sorted({v for p in o.data.polygons for v in p.vertices});vals=[[o.data.vertices[i].co[a] for i in used] for a in range(3)];sp=[max(x)-min(x) for x in vals];va=max(range(3),key=lambda a:sp[a]);rest=[a for a in range(3) if a!=va];la=max(rest,key=lambda a:sp[a]);da=[a for a in rest if a!=la][0];cen=[(min(vals[a])+max(vals[a]))/2 for a in range(3)];hi=min(vals[va])+.88*sp[va];hd=[o.data.vertices[i].co[da] for i in used if o.data.vertices[i].co[va]>=hi];fs=1 if not hd or max(hd)-cen[da]>=cen[da]-min(hd) else -1
...
def cls(c,b,mode):
 mn,mx,ct=b;h=(c.z-mn[2])/max(mx[2]-mn[2],1e-9);l=(c.x-ct[0])/max((mx[0]-mn[0])/2,1e-9);d=(c.y-ct[1])/max((mx[1]-mn[1])/2,1e-9);a=abs(l);f=d>=0;s=lambda L,R:L if l<0 else R
 if mode in ('D2','F2'):
  w=1 if mode=='D2' else 1.25
  if .69-(w-1)*.015<h<.80+(w-1)*.015 and .22-(w-1)*.03<a<.52+(w-1)*.03:return 101,.42
  if not f and .56-(w-1)*.02<h<.70+(w-1)*.02 and a<.48+(w-1)*.05:return 102,.45
  if .39-(w-1)*.02<h<.48+(w-1)*.02 and a<.42+(w-1)*.05:return 103,.44
  if .18-(w-1)*.015<h<.25+(w-1)*.015 and a>.10-(w-1)*.03:return 104,.50
  if .37-(w-1)*.015<h<.45+(w-1)*.015 and a>.52-(w-1)*.04:return 105,.48
 if h>.89:r=(1,.94)
 elif h>.82 and a<.25:r=(s(2,3),.82)
 elif .45<h<.82 and a>.48:
  r=(s(5,6),.79) if h>.70 else ((s(16,17),.76) if f else (s(18,19),.76)) if h>.57 else ((s(20,21),.72) if f else (s(22,23),.72))
 elif h>.70:r=(s(5,6),.78) if a>.30 else ((s(7,8),.82) if f else (4,.84))
 elif h>.50:r=((9,.86) if a<.17 else (s(10,11),.78)) if f else ((42,.72) if a<.16 and h<.61 else (s(12,13),.82))
 elif h>.40:r=((43,.70) if a<.18 else (s(26,27),.73)) if f else (s(14,15),.85)
 elif h>.24:r=(s(32,33),.82) if not f else ((s(30,31),.70) if a<.19 else (s(28,29),.82))
 elif h>.18:r=(s(34,35),.76)
 elif h>.055:r=(s(36,37),.74) if f else (s(38,39),.82)
 else:r=(s(40,41),.86)
 return (REM.get(r[0],r[0]),min(.95,r[1]+.03)) if mode=='E2' else r

def classify(o,mode):
 b=bounds(o.data);L=[];C=[];co=Counter()
 for p in o.data.polygons:
  x,c=cls(p.center,b,mode);L.append(x);C.append(c);co[x]+=1
 ...
 while 1:
  for yy in range(max(0,y0-w),min(s,y0+w+1)):
   for xx in range(max(0,x0-w),min(s,x0+w+1)):
    i=(yy*s+xx)*4;pix[i:i+4]=array('f',col)
...
def texture(o,L,path):
 ...
 for p in m.polygons:
  ...
  for j,a in enumerate(z):
   b=z[(j+1)%len(z)];v1=m.loops[a].vertex_index;v2=m.loops[b].vertex_index;k=tuple(sorted((v1,v2)));E[k].append((p.index,uv.data[a].uv.copy(),uv.data[b].uv.copy()))
 pix=array('f',[.94,.94,.94,1])*(SIZE*SIZE);n=0;un=0
 for vv in E.values():
  labs={L[x[0]] for x in vv}
  if len(labs)<2:continue
  n+=1;bad=any(x>=100 for x in labs);un+=bad;c=(.95,.06,.06,1) if bad else (.03,.03,.03,1)
  for _,a,b in vv:line(pix,SIZE,a,b,c,2 if bad else 1)
 im=bpy.data.images.new(path.stem,width=SIZE,height=SIZE,alpha=True);im.pixels.foreach_set(pix);im.filepath_raw=str(path);im.file_format='PNG';im.save();return {'status':'ORIGINAL_OBJ_UV','boundary_edges':n,'uncertain_edges':un}
def texmat(p):
...
def main():
 I=Path(arg('--input'));O=Path(arg('--output'));[ (O/x).mkdir(parents=True,exist_ok=True) for x in ('textures','screenshots','models','reports') ];H=sha(I);before=set(bpy.data.objects);bpy.ops.wm.obj_import(filepath=str(I));src=max([x for x in bpy.data.objects if x not in before and x.type=='MESH'],key=lambda x:len(x.data.polygons));src.name='CC0_SOURCE_REFERENCE';src.hide_render=True;src.hide_set(True);cs=comp(src.data);ref=clean(src,cs[0]);plan=norm(ref);ref.hide_render=True;ref.hide_set(True)
 S={'schema_version':'bp3d_online_wholebody_experiment_v1','status':'RUNNING','source_sha256':H,'public_cc0_only':True,'user_assets_loaded':False,'source_master_modified':False,'source_mesh':{'vertices':len(src.data.vertices),'polygons':len(src.data.polygons),'uv_layers':[u.name for u in src.data.uv_layers]},'component_sizes':[len(x) for x in cs[:20]],'clean_face_count':len(cs[0]),'axis_plan':plan,'modes':{},'errors':[]};ok=0
 for mode in MODES:
  made=[]
  try:
   w=ref.copy();w.data=ref.data.copy();bpy.context.scene.collection.objects.link(w);w.hide_set(False);w.hide_render=False;made.append(w);L,C,co=classify(w,mode);fa=w.data.attributes['BP3D_SOURCE_FACE_ID'];js(O/'reports'/(mode+'_face_labels.json'),{'mode':mode,'faces':[{'clean_face_index':i,'source_face_index':fa.data[i].value,'label_id':L[i],'label_key':N[L[i]],'confidence':C[i]} for i in range(len(L))]});tp=O/'textures'/(mode+'_boundary_lines.png');tr=texture(w,L,tp)
   if tr['status']=='NO_UV':
    bpy.ops.object.select_all(action='DESELECT');w.select_set(True);bpy.context.view_layer.objects.active=w;bpy.ops.object.mode_set(mode='EDIT');bpy.ops.mesh.select_all(action='SELECT');bpy.ops.uv.smart_project();bpy.ops.object.mode_set(mode='OBJECT');tr=texture(w,L,tp);tr['status']='SMART_PROJECT_FALLBACK'
   render(w,O/'screenshots'/(mode+'_REGION_FRONT.png'));render(w,O/'screenshots'/(mode+'_REGION_BACK.png'),True);t=w.copy();t.data=w.data.copy();bpy.context.scene.collection.objects.link(t);made.append(t);t.data.materials.clear();t.data.materials.append(texmat(tp));[setattr(p,'material_index',0) for p in t.data.polygons];render(t,O/'screenshots'/(mode+'_TEXTURE_APPLY_FRONT.png'));t.hide_render=True;t.hide_set(True);xs=split(w,mode);made+=xs;bpy.ops.wm.save_as_mainfile(filepath=str(O/'models'/(mode+'_SPLIT.blend')),copy=True)
   try:
    bpy.ops.object.select_all(action='DESELECT');[x.select_set(True) for x in xs];bpy.context.view_layer.objects.active=xs[0];bpy.ops.export_scene.gltf(filepath=str(O/'models'/(mode+'_SPLIT.glb')),export_format='GLB',use_selection=True)
   except Exception as e:S['errors'].append({'mode':mode,'stage':'GLB_EXPORT','error':str(e)})
   S['modes'][mode]={'status':'SUCCESS','label_count':len(co),'face_count':len(L),'mean_confidence':sum(C)/len(C),'uncertainty_faces':sum(v for k,v in co.items() if k>=100),'counts':dict(co),'texture':tr,'split_object_count':len(xs)};ok+=1
 ...
 S['successful_mode_count']=ok;S['status']='PASS' if ok==3 else ('PARTIAL' if ok else 'FAIL');js(O/'summary.json',S);(O/'SUCCESS_MARKER.txt').write_text(S['status']+'\n');print(json.dumps({'status':S['status'],'modes':ok,'clean_faces':len(cs[0]),'uv_layers':S['source_mesh']['uv_layers']},indent=2));
 if ok<3:raise RuntimeError('Only %d/3 modes succeeded'%ok)
if __name__=='__main__':
 try:main()
 except Exception as e:
  try:O=Path(arg('--output'));(O/'reports').mkdir(parents=True,exist_ok=True);js(O/'reports'/'FATAL_ERROR.json',{'error':str(e),'traceback':traceback.format_exc()})
  except:pass
  raise
