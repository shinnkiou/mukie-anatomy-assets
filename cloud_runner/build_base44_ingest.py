...
def aggregate(mode, summary, face_doc):
    ...
    for zone in ZONE_IDS:
        ...
        motion_bad=max((float(m.get("bad_pct",0) or 0) for m in final.get("motion",[]) if zone in MOTION_ZONES.get(m.get("motion"),[])),default=0.0)
...
def main(root):
    ...
    for mode in ("SAFE","BALANCED","FINE"):
        ...
        for m in summary["final"][mode].get("motion",[]):
            name=m.get("motion")
    ...
    out={"schema_version":"bp3d_base44_ingest_records_v1","source_unchanged":summary.get("source_unchanged"),"source_master_modified":summary.get("source_master_modified"),"abc_abcd_modified":summary.get("abc_abcd_modified"),"zone_records":records,"motion_records":motions}
